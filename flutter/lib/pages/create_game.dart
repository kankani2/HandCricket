...
class _CreateGamePage extends State<CreateGamePage> {
  ...
  void moveToTeamMatch() async {
    ...
    var response = await request(HttpMethod.POST,
        "/game/${gameInfo.gameID}/match", {"gameCode": gameInfo.gameCode});
  ...
  Future<User> getUserInfoFromID(String uid) async {
  ...
  void addListenerForGameToUpdatePlayers(GameInfo game) {
    _gamesRef
        ...
        .onChildAdded
        .listen((event) async {
      User playerInfo = await getUserInfoFromID(event.snapshot.key);
      setState(() {
